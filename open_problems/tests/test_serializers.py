...
class OPSerializerTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.john_doe = Contact.objects.create(
            first_name="John", last_name="Doe", email="john_doe@test.com"
        )
        cls.jane_doe = Contact.objects.create(
            first_name="Jane", last_name="Doe", email="jane_doe@test.com"
        )

        # Create a sample OpenProblems instance
        cls.problem = OpenProblem.objects.create(
            title="Test Problem",
        ...
        cls.child_problem = OpenProblem.objects.create(
    ...
    def test_serializer_with_children(self):
        ...
        self.assertEqual(
            serialized_data["contact"], self.problem.contact.id
        )  # Use self.problem.contact.id
        self.assertIsNone(
            serialized_data["parent_problem"]
        )  # No parent_problem for root problem
        self.assertEqual(
            len(serialized_data["children"]), 1
        )  # No children for root problem
